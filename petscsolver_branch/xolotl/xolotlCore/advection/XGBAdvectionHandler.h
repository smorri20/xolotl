		...
		for (int i = 0; i < nAdvec; i++) {
			...
			if (isPointOnSink(pos)) {
				...
				double conc = (3.0 * sinkStrengthVector[i] * cluster->getDiffusionCoefficient())
							* ((oldLeftConc / pow(hxLeft, 5)) + (oldRightConc / pow(hxRight, 5)))
				...
				double b = abs(location - pos[0]) + hxRight*(pos[0] > location) + hxLeft*(pos[0] < location);

				// Compute the concentration as explained in the description of the method
				double conc = (3.0 * sinkStrengthVector[i] * cluster->getDiffusionCoefficient())
							* ((oldRightConc / pow(b, 4)) - (oldConc / pow(a, 4)))
							/ (xolotlCore::kBoltzmann * cluster->getTemperature()
									* (hxRight*(pos[0] > location) + hxLeft*(pos[0] < location)));
	...
	void computePartialsForAdvection(PSIClusterReactionNetwork *network,
		...
		for (int i = 0; i < nAdvec; i++) {
			...
			if (isPointOnSink(pos)) {
				...
				val[i * 2] = -(3.0 * sinkStrength * diffCoeff)
							/ (xolotlCore::kBoltzmann * cluster->getTemperature() * pow(a, 4)
									* (hxRight*(pos[0] > location) + hxLeft*(pos[0] < location))); // middle
				val[(i * 2) + 1] = (3.0 * sinkStrength * diffCoeff)
							/ (xolotlCore::kBoltzmann * cluster->getTemperature() * pow(b, 4)
									* (hxRight*(pos[0] > location) + hxLeft*(pos[0] < location))); // left or right
			}
		}
