	...
	for (PetscInt yj = bottomOffset; yj < nY - topOffset; yj++) {
		...
		for (int xi = surfacePosition[yj] + leftOffset; xi < nX - rightOffset;
			...
			if (xi >= xs && xi < xs + xm && yj >= ys && yj < ys + ym) {
				...
				atomConc += network.getTotalTrappedAtomConcentration(concs[yj][xi], 0)
						* (grid[xi + 1] - grid[xi]);
			}
		}
		...
		for (PetscInt xi = xs; xi < xs + xm; xi++) {
			...
			network.computeAllFluxes(concOffset, xi - xs, updatedConcOffset);
		}
	}
	...
	for (PetscInt yj = bottomOffset; yj < nY - topOffset; yj++) {
		...
		for (PetscInt xi = xs; xi < xs + xm; xi++) {
			...
			int nMutating = mutationHandler->computePartialsForTrapMutation(
					network, concOffset, mutationVals, mutationIndices, xi, xs, yj);
