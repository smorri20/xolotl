...
PetscErrorCode computeCumulativeHelium1D(TS ts, PetscInt timestep, PetscReal time,
	...
	for (int xi = xs; xi < xs + xm; xi++) {
		...
		for (unsigned int i = 0; i < heIndices1D.size(); i++) {
			// Add the current concentration times the number of helium in the cluster
			// (from the weight vector)
			heTotalConcentration += gridPointSolution[heIndices1D[i]] * heWeights1D[i]
			                                     * (grid[xi] - grid[xi-1]);
		}
	}
	...
	// Sum all the concentrations through MPI reduce
	double totalHeConcentration = 0.0;
	MPI_Reduce(&heTotalConcentration, &totalHeConcentration, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

	// Store the concentration over the grid points
	...
	for (int xi = 0; xi < xSize; xi++) {
		...
		if (procId == 0) {
			...
			outputFile << x - grid[surfacePos] << " " << heConcentration / totalHeConcentration << std::endl;
		}
	}
	...
	if (procId == 0) {
		...
		for (xi = xs; xi < xs + xm; xi++) {
			...
			for (int i = 0; i < loopSize; i++) {
				...
				// and add it to myPoints
				xolotlViz::Point aPoint;
				aPoint.value = gridPointSolution[i + 6];
				aPoint.t = time;
				aPoint.x = grid[xi];
		...
		for (int i = 0; i < loopSize; i++) {
			auto cluster = (PSICluster *) reactants->at(i + 6);
			// Get the data provider and give it the points
		...
		for (xi = xs; xi < xs + xm; xi++) {
			...
			for (int i = 0; i < loopSize; i++) {
				...
				MPI_Send(&gridPointSolution[i + 6], 1, MPI_DOUBLE, 0, 22,
						MPI_COMM_WORLD);
			}
		}
	}
