...
void Diffusion3DHandler::computeDiffusion(IReactionNetwork *network,
		...
		double sy, int iy, double sz, int iz) const {
    ...
    for (IReactant const& currReactant : diffusingClusters) {
		...
		double oldConc = concVector[0][index] * diffusionGrid[iz+1][iy+1][ix+1][diffClusterIdx]; // middle
		double oldLeftConc = concVector[1][index] * diffusionGrid[iz+1][iy+1][ix][diffClusterIdx]; // left
		double oldRightConc = concVector[2][index] * diffusionGrid[iz+1][iy+1][ix+2][diffClusterIdx]; // right
		double oldBottomConc = concVector[3][index] * diffusionGrid[iz+1][iy][ix+1][diffClusterIdx]; // bottom
		double oldTopConc = concVector[4][index] * diffusionGrid[iz+1][iy+2][ix+1][diffClusterIdx]; // top
		double oldFrontConc = concVector[5][index] * diffusionGrid[iz][iy+1][ix+1][diffClusterIdx]; // front
		double oldBackConc = concVector[6][index] * diffusionGrid[iz+2][iy+1][ix+1][diffClusterIdx]; // back
...
void Diffusion3DHandler::computePartialsForDiffusion(
    ...
    for (IReactant const& currReactant : diffusingClusters) {
		...
		val[diffClusterIdx * 7] = - 2.0 * diffCoeff * ((1.0 / (hxLeft * hxRight)) + sy + sz)
				* diffusionGrid[iz+1][iy+1][ix+1][diffClusterIdx]; // middle
		val[(diffClusterIdx * 7) + 1] = diffCoeff * 2.0 / (hxLeft * (hxLeft + hxRight))
				* diffusionGrid[iz+1][iy+1][ix][diffClusterIdx]; // left
		val[(diffClusterIdx * 7) + 2] = diffCoeff * 2.0 / (hxRight * (hxLeft + hxRight))
				* diffusionGrid[iz+1][iy+1][ix+2][diffClusterIdx]; // right
		val[(diffClusterIdx * 7) + 3] = diffCoeff * sy * diffusionGrid[iz+1][iy][ix+1][diffClusterIdx]; // bottom
		val[(diffClusterIdx * 7) + 4] = diffCoeff * sy * diffusionGrid[iz+1][iy+2][ix+1][diffClusterIdx]; // top
		val[(diffClusterIdx * 7) + 5] = diffCoeff * sz * diffusionGrid[iz][iy+1][ix+1][diffClusterIdx]; // front
		val[(diffClusterIdx * 7) + 6] = diffCoeff * sz * diffusionGrid[iz+2][iy+1][ix+1][diffClusterIdx]; // back
