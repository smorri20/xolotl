...
/**
 * Computes Pade function in 2D given the coefficients and orders a and b
 */
inline double pade(int a, int b, std::vector<double> & coef, double x,
		double y) {
	...
	// Loop on a
	for (int i = 0; i <= a; i++) {
		for (int iBis = 0; iBis <= i; iBis++) {
			numerator += coef[index] * pow(x, (double) iBis)
					* pow(y, (double) (i - iBis));
			index++;
		}
	}
	...
	for (int i = 1; i <= b; i++) {
		for (int iBis = 0; iBis <= i; iBis++) {
			denominator += coef[index] * pow(x, (double) iBis)
					* pow(y, (double) (i - iBis));
			index++;
		}
	}
