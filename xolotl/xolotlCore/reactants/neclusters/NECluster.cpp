...
void NECluster::resetConnectivities() {
	...
	std::for_each(combiningReactants.begin(), combiningReactants.end(),
			[this](const CombiningCluster& cc) {
				...
				setReactionConnectivity(combCluster.getId());
				setReactionConnectivity(combCluster.getMomentId(0));
			});
			...
			std::accumulate(dissociatingPairs.begin(), dissociatingPairs.end(),
                ...
                [&concs,xi](double running, const ClusterPair& currPair) {
					...
					return running + (currReaction.kConstant[xi] *
							dissociatingCluster->getConcentration(concs, currPair.firstDistance));
				});
}
...
void NECluster::getProductionFlux(const double* concs, int xi,
	...
	std::for_each(reactingPairs.begin(), reactingPairs.end(),
			[&concs,xi,&flux](ClusterPair const& currPair) {
				// Get the two reacting clusters
				NECluster* firstReactant = currPair.first;
				NECluster* secondReactant = currPair.second;
				// Update the flux
				Reaction const& currReaction = currPair.reaction;
				flux.flux += currReaction.kConstant[xi]
				* firstReactant->getConcentration(concs, currPair.firstDistance)
				* secondReactant->getConcentration(concs, currPair.secondDistance);
			});
}
...
void NECluster::getCombinationFlux(const double* concs, int xi,
	...
	flux.flux = std::accumulate(combiningReactants.begin(),
			combiningReactants.end(),
            0.0,
			[&concs,xi](double running, const CombiningCluster& currPair) {
				...
				(currReaction.kConstant[xi] *
						combiningCluster.getConcentration(concs, currPair.distance));
	...
	std::for_each(reactingPairs.begin(), reactingPairs.end(),
			[&concs,&partials,xi](ClusterPair const& currPair) {
				...
				* currPair.second->getConcentration(concs,
				...
				auto index = currPair.first->getId() - 1;
				partials[index] += value;
				index = currPair.first->getMomentId(0) - 1;
				...
				* currPair.first->getConcentration(concs,
				...
				index = currPair.second->getId() - 1;
				partials[index] += value;
				index = currPair.second->getMomentId(0) - 1;
...
void NECluster::getCombinationPartialDerivatives(const double* concs, int xi,
	...
	std::for_each(combiningReactants.begin(), combiningReactants.end(),
			[this,&concs,&partials,xi](const CombiningCluster& cc) {
				...
				partials[getId() - 1] -= currReaction.kConstant[xi] *
				cluster.getConcentration(concs, cc.distance);
				// Compute the contribution from the combining cluster
				double value = currReaction.kConstant[xi] * getConcentration(concs);

				partials[cluster.getId() - 1] -= value;
				partials[cluster.getMomentId(0) - 1] -= value * cc.distance;
			});
...
void NECluster::getDissociationPartialDerivatives(const double* concs, int xi,
	...
	std::for_each(dissociatingPairs.begin(), dissociatingPairs.end(),
			[&concs,&partials,xi](const ClusterPair& currPair) {
				...
				Reaction const& currReaction = currPair.reaction;
				partials[cluster->getId() - 1] += currReaction.kConstant[xi];
				partials[cluster->getMomentId(0) - 1] += currReaction.kConstant[xi] *
				currPair.firstDistance;
			});
