	...
	if (size < 3) {
		...
		for (unsigned int i = 0; i < heVReactants.size(); i++) {
			...
			for (unsigned int j = 0; j < heReactants.size(); j++) {
				...
				if (otherProduct
						&& (heVCluster->getDiffusionFactor() > 0.0
								|| heCluster->getDiffusionFactor() > 0.0)) {
					// The reaction is really allowed
					// Create the corresponding production reaction
					auto reaction = std::make_shared<ProductionReaction>(
							heVCluster, heCluster);
					// Add it to the network
					reaction = network->addProductionReaction(reaction);
					// Create the cluster pair
					ClusterPair pair(heVCluster, heCluster, reaction.get());
					// Add the pair to the list
					reactingPairs.push_back(pair);
					// Setup the connectivity array
		...
		for (unsigned int i = 0; i < heReactants.size(); i++) {
			...
			for (unsigned int j = firstSize - 1; j < heReactants.size(); j++) {
				...
				if (size == 1) {
					...
					smallerProduct = (PSICluster *) network->getCompound(
							heVType, comp);
				}
				if (smallerProduct)
					continue;
				...
				auto otherProduct = network->getCompound(heVType, comp);
				if (otherProduct
						&& (firstCluster->getDiffusionFactor() > 0.0
					...
					reaction = network->addProductionReaction(reaction);
					// Create the cluster pair
					ClusterPair pair(firstCluster, secondCluster,
							reaction.get());
					// Add the pair to the list
					reactingPairs.push_back(pair);
					// Setup the connectivity array
