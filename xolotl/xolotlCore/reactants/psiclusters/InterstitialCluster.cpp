...
InterstitialCluster::InterstitialCluster(int nI,
	...
	reactionRadius = xolotlCore::tungstenLatticeConstant * pow((3.0 / EightPi) * size, (1.0 / 3.0));
...
//	double flux = PSICluster::getEmissionFlux();
	...
	if (size < 2) {
		// bias * k^2 * D * C
		flux += 1.05 * 0.00005 * diffusionCoefficient * concentration;
	}
...
//		PSICluster::getEmissionPartialDerivatives(partials);
		...
		if (size < 2) {
			// bias * k^2 * D * C
			partials[id - 1] -= 1.05 * 0.00005 * diffusionCoefficient;
		}
