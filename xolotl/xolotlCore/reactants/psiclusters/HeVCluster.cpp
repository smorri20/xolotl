...
void HeVCluster::combineClusters(std::vector<IReactant *> & clusters,
	...
	for (unsigned int i = 0; i < clusters.size(); i++) {
		...
		if (firstProduct && secondProduct
				&& (diffusionFactor > 0.0
						|| secondCluster->getDiffusionFactor() > 0.0)) {
...
void HeVCluster::createReactionConnectivity() {
	...
	if (singleVReactant && secondReactant
			&& (singleVReactant->getDiffusionFactor() > 0.0
					|| secondReactant->getDiffusionFactor() > 0.0)) {
		// Create the corresponding production reaction
		auto reaction = std::make_shared<ProductionReaction>(singleVReactant,
				secondReactant);
		// Add it to the network
		reaction = network->addProductionReaction(reaction);
		// Create the cluster pair
		ClusterPair pair(singleVReactant, secondReactant, reaction.get());
		// Add the pair to the list
		reactingPairs.push_back(pair);
		// Setup the connectivity array
	...
	if (heliumReactant && vacancyReactant
		...
		reaction = network->addProductionReaction(reaction);
		// Create the cluster pair
		ClusterPair pair(heliumReactant, vacancyReactant, reaction.get());
		// Add the pair to the list
		reactingPairs.push_back(pair);
		// Setup the connectivity array
		int Id = heliumReactant->getId();
	...
	for (unsigned int i = 0; i < reactants.size(); i++) {
		...
		if (secondReactant
			...
			auto reaction = std::make_shared<ProductionReaction>(
					interstitialReactant, secondReactant);
			// Add it to the network
			reaction = network->addProductionReaction(reaction);
			// Create the cluster pair
		...
		for (unsigned int i = 0; i < reactants.size(); i++) {
			...
			if (otherReactant && iCluster
					&& (heReactant->getDiffusionFactor() > 0.0
							|| otherReactant->getDiffusionFactor() > 0.0)) {
				// The reaction is really allowed
				// Create the corresponding production reaction
				auto reaction = std::make_shared<ProductionReaction>(heReactant,
						otherReactant);
				// Add it to the network
				reaction = network->addProductionReaction(reaction);
				// Create the cluster pair
				ClusterPair pair(heReactant, otherReactant, reaction.get());
				// Add the pair to the list
				reactingPairs.push_back(pair);
				// Setup the connectivity array
