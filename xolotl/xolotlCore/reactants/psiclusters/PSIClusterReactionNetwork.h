...
/**
 * This class provides a hashing function for the PSIClusterReactionNetwork and
 * its unordered maps.
 */
class CompositionHasher {
public:
	/**
	 ...
	 * @return The hash
	 */
	long operator()(const std::unordered_map<std::string, int>& composition) const {
		// I pulled 1e9 out of my posterior. I just wanted a really big
		// hash table.
		int bigNumber = 1e9;
...
/**
 * This class provides an equality checking function for the
 * PSIClusterReactionNetwork and its unordered maps.
 */
class CompositionEqualityChecker {
  public:
	/**
	 * The equality operation.
	 * @param compA The first composition to compare to the second.
	 * @param compB The second composition.
	 * @return True if the compositions are equal, false if not
	 */
     bool operator()(const std::unordered_map<std::string,int> & compA,
    		 const std::unordered_map<std::string,int>& compB) const
     {
        return (compA.at("He") == compB.at("He"))
        		&& (compA.at("V") == compB.at("V"))
        		&& (compA.at("I") == compB.at("I"));
     }
};
...
private:
	 ...
	 */
	std::unordered_map<std::unordered_map<std::string, int>, std::shared_ptr<PSICluster>,
			xolotlCore::CompositionHasher, xolotlCore::CompositionEqualityChecker> singleSpeciesMap;
	 ...
	 */
	std::unordered_map<std::unordered_map<std::string, int>, std::shared_ptr<PSICluster>,
			xolotlCore::CompositionHasher, xolotlCore::CompositionEqualityChecker> mixedSpeciesMap;
