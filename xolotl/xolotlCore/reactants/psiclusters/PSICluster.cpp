...
double PSICluster::getDissociationFlux(double temperature) {
...
double PSICluster::getProductionFlux(double temperature) {
...
double PSICluster::getCombinationFlux(double temperature) {
...
double PSICluster::getDiffusionCoefficient(double temperature) {
	...
	if (!equal(temperature, lastDiffusionTemperature) || !equal(migrationEnergy,lastMigrationEnergy)) {
		...
		// Reset the cached temperature and diffusion coefficient
		lastDiffusionTemperature = temperature;
		lastDiffusionCoefficient = diffusionFactor * exp(kernel);
	}

	return lastDiffusionCoefficient;
}
...
void PSICluster::setMigrationEnergy(const double energy) {
	// Set the migration energy and cache its previous value.
	lastMigrationEnergy = migrationEnergy;
	migrationEnergy = energy;
	return;
}

double PSICluster::calculateReactionRateConstant(PSICluster & firstReactant,
		PSICluster & secondReactant, double temperature) {
...
double PSICluster::calculateDissociationConstant(PSICluster & firstCluster,
...
void PSICluster::getCombinationPartialDerivatives(
		std::vector<double> & partials, double temperature) {
 ...
 */
std::vector<double> PSICluster::getPartialDerivatives(double temperature) {
