...
void HeInterstitialCluster::replaceInCompound(
	...
	for (unsigned int i = 0; i < reactants.size(); i++) {
		...
		std::vector<int> productCompositionVector = {
				productReactantComp[heType], 0, productReactantComp[iType] };
...
void HeInterstitialCluster::createReactionConnectivity() {
	...
	for (unsigned int i = 0; i < reactants.size(); i++) {
		...
		if (secondReactant
				&& (heliumReactant->getDiffusionFactor() > 0.0
						|| secondReactant->getDiffusionFactor() > 0.0)) {
			// Create the corresponding production reaction
			auto reaction = std::make_shared<ProductionReaction>(heliumReactant,
					secondReactant);
			// Add it to the network
			reaction = network->addProductionReaction(reaction);
			// Create the cluster pair
			ClusterPair pair(heliumReactant, secondReactant, reaction.get());
	...
	if (singleIReactant && secondReactant
			&& (singleIReactant->getDiffusionFactor() > 0.0
					|| secondReactant->getDiffusionFactor() > 0.0)) {
		// Create the corresponding production reaction
		auto reaction = std::make_shared<ProductionReaction>(singleIReactant,
				secondReactant);
		// Add it to the network
		reaction = network->addProductionReaction(reaction);
		// Create the cluster pair
		ClusterPair pair(singleIReactant, secondReactant, reaction.get());
		// Add the pair to the list
		reactingPairs.push_back(pair);
		// Setup the connectivity array
	...
	if (heliumReactant && interstitialReactant
			&& (heliumReactant->getDiffusionFactor() > 0.0
					|| interstitialReactant->getDiffusionFactor() > 0.0)) {
		// Create the corresponding production reaction
		auto reaction = std::make_shared<ProductionReaction>(heliumReactant,
				interstitialReactant);
		// Add it to the network
		reaction = network->addProductionReaction(reaction);
		// Create the cluster pair
		ClusterPair pair(heliumReactant, interstitialReactant, reaction.get());
		// Add the pair to the list
		reactingPairs.push_back(pair);
		// Setup the connectivity array
		int id = heliumReactant->getId();
	...
	for (unsigned int i = 0; i < reactants.size(); i++) {
		...
		// Create a ReactingPair with the two reactants if they both exist
		if (secondReactant
				&& (vacancyReactant->getDiffusionFactor() > 0.0
						|| secondReactant->getDiffusionFactor() > 0.0)) {
			// Create the corresponding production reaction
			auto reaction = std::make_shared<ProductionReaction>(
					vacancyReactant, secondReactant);
			// Add it to the network
			reaction = network->addProductionReaction(reaction);
			// Create the cluster pair
			ClusterPair pair(vacancyReactant, secondReactant, reaction.get());
			// Add the pair to the list
			reactingPairs.push_back(pair);
			// Setup the connectivity array
...
void HeInterstitialCluster::createDissociationConnectivity() {
	...
	auto heIClusterLessHe = (PSICluster *) network->getCompound(typeName,
			compositionVec);
	...
	auto heIClusterMoreHe = (PSICluster *) network->getCompound(typeName,
			compositionVec);
	...
	auto heIClusterLessI = (PSICluster *) network->getCompound(typeName,
			compositionVec);
	//Get the single interstitial cluster
	...
	compositionVec = {numHe, 0, numI + 1};
	auto heIClusterMoreI = (PSICluster *) network->getCompound(typeName,
			compositionVec);
