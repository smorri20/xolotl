	...
	public void testSpecifiedArguments() {
		...
		try {
			...
			args = CliFactory.parseArguments(Arguments.class, new String[] {
					...
					"dummy", "--dimensions", "2", "--maxHeSize", "7", 
					"--maxVSize", "30", "--maxISize", "5", "--checkpoint", "xolotlStop.h5", 
					"--stepSize", "3.0", "--initialV", "0.05", "--voidPortion", "60.0", 
					"--process", "none"});
			...
			// Check if there is a physics process argument
			assertEquals(true, args.isProcess());

			// Check its value
			assertEquals("none", args.getProcess());
